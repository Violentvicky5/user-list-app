...
export default function WorkTablePage() {
  ...
  const fetchWorks = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.error || "Failed to fetch work data");
  ...
  return (
    <div className="max-w-6xl mx-auto mt-10 p-4 text-black dark:text-white">
      ...
      {loading && <p className="text-sm text-gray-500 dark:text-gray-400">Loading works…</p>}
        ...
        <table className="min-w-full bg-white dark:bg-gray-800">
          ...
          <tbody className="text-sm">
            {!works.length && !loading ? (
              ...
              works.map((u) => (
                <tr key={u.userId} className="border-t border-gray-200 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700">
                  <td className="px-4 py-3">{u.userId}</td>
                  <td className="px-4 py-3">{u.username}</td>
                  <td className="px-4 py-3">{u.works.map((w) => w.name).join(", ")}</td>
